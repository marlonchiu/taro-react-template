...
export default function useRegion(props?: Props) {
  ...
  const { isGetAddress = true } = props || {};

  const { address, SET_STATE } = useAppStore(useSelector(['address', 'SET_STATE']));
   ...
   */
  async function getRegion(failTips = false, forceAddress = false) {
